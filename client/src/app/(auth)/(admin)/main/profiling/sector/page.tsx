  ...
  const handleRowClick = async (purok: PurokData) => {
    ...
    try {
      const response = await api.get(
        `/api/sectoral-by-purok?sitio_purok=${purok.sitio_purok}`
      );
      const filteredData = response.data.find(
        (item: { purok_id: number }) => item.purok_id === purok.purok_id
      );
      ...
      if (filteredData) {
        ...
        const populationResponse = await api.get(
          `/api/purok-population?sitio_purok=${purok.purok_name}`
        );
        const populationData = populationResponse.data.find(
          (item: { sitio_purok: string }) =>
            item.sitio_purok === purok.purok_name
        );
        ...
        <div className="h-full w-[50%] bg-white">
          <div className="w-full h-full flex justify-center py-10 relative">
            ...
            <div className="absolute bottom-0 left-0 p-4 text-[14px]">
              Total Population:{" "}
              {totalPopulation !== null ? totalPopulation : "N/A"}
            </div>
            <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 p-4 text-[14px]">
              Purok {selectedPurokId}:{" "}
              {
                purokData.find((purok) => purok.purok_id === selectedPurokId)
                  ?.purok_name
              }
            </div>
          </div>
