...
const Households = () => {
  ...
  const handleSearch = debounce(async (searchTerm: string) => {
    ...
    try {
      ...
      const response = await api.get(`/api/resident/search?term=${searchTerm.trim()}`);
  ...
  const HEADER = [
    "Household Number", isSearching ? "Household" : "Household Head", "Purok", "BUSINESS OWNER"
  ]
  ...
  return (
    <div className="h-full w-full">
      ...
      <ProfilingSearchBar onSearch={handleSearch} setAddResidentModal={() => setAddResidentModal(true)} isResident={true} />
            ...
            <thead>
              <tr className="sticky top-0 bg-white shadow-gray-300 shadow-sm z-10">
                {HEADER.map((item, index) => (
                  <th key={index} className="text-left py-5 font-semibold px-20">
                    {item}
                  </th>
                ))}
              </tr>
            ...
            {loading ? (
              ...
              <tbody>
                {displayData.map((item) => (
                  ...
                  >
                    <td className="py-2 text-left px-20">{item.household_number}</td>
                    <td className="py-2 text-left px-20">
                      {item.given_name} {item.middle_name} {item.family_name} {item.extension}
                    </td>
                    ...
                    <td className="py-2 text-left px-20">
                      <p
                        className={`${item.is_business_owner === "Yes"
                          ...
                          : "bg-[#A4A4A4]"} rounded-[4px] py-1 w-20 inline-block text-center`}
                      >
              ...
              <tbody>
                <tr>
                  <td colSpan={HEADER.length} className="py-10 text-center text-gray-500">
                    {isSearching ? "No residents found. Try adjusting your search." : "No household heads available."}
                  </td>
                </tr>
              </tbody>
            )}
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      {...register("household_number", {
                        required: true,
                        validate: (value) => {
                          const exists = householdHeads.some(householdHead =>
